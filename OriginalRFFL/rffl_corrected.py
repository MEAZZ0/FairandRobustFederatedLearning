...
from utils.utils import compute_grad_update, add_update_to_model, add_gradient_updates,\
	flatten, unflatten, train_model, evaluate, cosine_similarity, mask_grad_update_by_order, tmn
...
parser.add_argument('-atk', '--adversary_attack', help='The type of adversaries', type=str, default='lf', choices=['lf', 're', 'vi', 'sr', 'fr', 'all'])
		...
		for split in splits:
			...
			for _round in range(args['rounds']):
				...
				if not use_reputation:
					...
					#aggregated_gradient= tmn(gradients)
				...
				for i in range(N+A):
					...
					if i > A-1:
						add_update_to_model(participant_models[i-A], reward_gradient, device=device)#changed device
					else: # N<= i < N+A:
