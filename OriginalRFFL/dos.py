...
from utils.arguments import mnist_args, cifar_cnn_args, mr_args, sst_args, fmnist_args
from utils.utils import compute_grad_update, add_update_to_model, add_gradient_updates,\
	flatten, unflatten, train_model, evaluate, cosine_similarity, mask_grad_update_by_order, tmn, abnormalCompute
...
# The coefficient to calculate beta = threshold * (1.0/N). default=1.0/3.0
...
parser.add_argument('-atk', '--adversary_attack', help='The type of adversaries', type=str, default='lf', choices=['lf', 're', 'vi', 'sr', 'fr', 'alt', 'all'])
...
###
def scaleup1(grads,k,n):
    # Flatten the gradient tensor
    flat_grads = grads.view(-1)
    
    # Calculate the number of elements to flip (20% of total elements)
    num_elements = flat_grads.size(0)
    num_elements_to_flip = int(n * num_elements)

    # Find the indices of the lowest values
    _, indices = torch.topk(flat_grads, k=num_elements_to_flip, largest=True)
    meantsr= torch.mean(grads)
    # Create a mask to flip the lowest values
    mask = torch.zeros_like(flat_grads)
    mask.scatter_(0, indices, k)
    # Flip the sign of the lowest values+1*meantsr
    flipped_grads = (grads) * (-mask).view_as(grads) + grads * (1-mask).view_as(grads) #1-mask
    
    return flipped_grads
###
def scaleup(grads,k,n):
    # Flatten the gradient tensor
    flat_grads = grads.view(-1)
    ...
    mask.scatter_(0, indices, k)
    # Flip the sign of the lowest values+1*meantsr
    flipped_grads = (grads) * (-mask).view_as(grads) + grads * (mask-1).view_as(grads) #1-mask
	...
	for atk in atks:
		...
		for split in splits:
			...
			for i in range(N):
				...
				scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma = 1)#args['lr_decay']
			...
			for _round in range(args['rounds']):
				...
				for i in range(A):
					...
					if atk == 'fr':
						...
						if atk == 're':
							...
							#add_update_to_model(adv_models[i], adv_grad, device=device)
							np.random.seed(42)
							if _round == args['rounds'] -1:
								for grad in adv_grad:
									#grad.data *= torch.from_numpy(np.random.choice([-1, 1], p=[1,0], size=grad.shape)).to(grad.device)
									grad.data = scaleup(grad.data,1000000,0.02)
							...
							elif _round % 2==1 and _round>10:
							
								for grad in adv_grad:
									#grad.data *= torch.from_numpy(np.random.choice([-1, 1], p=[0.2,0.8], size=grad.shape)).to(grad.device)
									grad.data = scaleup1(grad.data,1000000,0.01)##0.01
								flattened = flatten(adv_grad)
								norm_value = (norm(flattened) + 1e-7)#*(5/(N+A))*(1/A) # to prevent division by zero
								adv_grad = unflatten(torch.multiply(torch.tensor(args['Gamma']), torch.div(flattened,  norm_value)), adv_grad)
									# (torch.rand(size=grad.shape, device=grad.device) - 0.5).sign()
							

					flattened = flatten(adv_grad)
					hist_shape1= adv_grad
					norm_value = norm(flattened) + 1e-7 # to prevent division by zero
					...
					if i in R_set:
						copod_gradients.append(adv_grad)#hist_shape1
				...
				for i in range(N):
					...
					if i+A in R_set:
						copod_gradients.append(gradient)#hist_shape
				...
				if not use_reputation:
					...
					for gradient, weight in zip(gradients, weights):
						add_gradient_updates(aggregated_gradient, gradient, weight=weight)
#
					aggregated_gradient, new_weights= abnormalCompute(gradients)#copod_gradients
					flat_aggre_grad = flatten(aggregated_gradient)
#
					norm_value1 = norm(flat_aggre_grad) + 1e-7
					flat_aggre_grad= torch.multiply(torch.tensor(1), torch.div(flat_aggre_grad,  1).to(device)) #args['Gamma'],norm_value1

					phis = torch.zeros( N + A, device=device)
					for i, gradient in enumerate(gradients):
						phis[i] = F.cosine_similarity(flatten(gradient), flat_aggre_grad, 0, 1e-10) #new_weights[i] #
				...
				for i in range(N+A):
					...
					if i > A-1:
						if _round > 10:
							newgrad =  unflatten(-rs[i]*flatten(gradients[i]).to(device)+flatten(reward_gradient).to(device),reward_gradient)
							#if _round % 2==0:
							#	newgrad =  unflatten(-q_ratio*flatten(gradients[A+i]).to(device)+q_ratio*flatten(reward_gradient).to(device),reward_gradient)#rs[A+i]
							#else:
							#	newgrad =  unflatten(-1*flatten(gradients[A+i]).to(device)+flatten(reward_gradient).to(device),reward_gradient)#rs[A+i]
						else:
							newgrad =  unflatten(-1*flatten(gradients[i]).to(device)+flatten(reward_gradient).to(device),reward_gradient)
						#add_gradient_updates(newgrad, gradients[A+i], weight=-rs[A+i])
						#add_gradient_updates(newgrad, reward_gradient, weight=rs[A+i])
						add_update_to_model(participant_models[i-A], newgrad, device=device)
					else: # N<= i < N+A:
						
						add_update_to_model(adv_models[i], reward_gradient, device=device)
